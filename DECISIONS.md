...
## 2026-09-25 — Phase 2 NUT-03 evidence correction (uncommitted, not yet run in CI)
...
**The fix:** each NUT-03 script now writes its own evidence file, at the moment it computes the values, from the same real database reads, HTTP responses and NUT-07 states its console output is built from (`src/cli/real-cashu/nut03-evidence.ts`). Nothing is rebuilt afterward from logs. Files: `nut03-swap.json`, `nut03-conservation.json`, `nut03-accounting-rows.json`, `nut03-receipts.json` (and `-post-restart` versions of all four), `nut03-failed-swap.json`, `nut03-double-spend.json`, `nut03-crash-before-commit.json`, `nut03-response-loss.json`, `nut03-restart.json`, `nut03-multi-swap.json`, `nut03-nut07-reconciliation.json`. The crash drill and restart are run from workflow bash, so two small scripts (`pol-swap-crash-evidence.ts`, `pol-swap-restart-evidence.ts`) take the row-count strings that bash already reads from the real database and write them as JSON. The misleading second NUT-04 generator step was replaced by a NUT-07 collation step and a gating verification step.

**NUT-07 strengthened:** the double-spend test now checks the first swap's replacements (UNSPENT) as well as the originals. The multi-swap test now checks, after each swap, that the consumed proofs are SPENT and the issued proofs are UNSPENT. The crash drill now checks its original proofs against the restarted mint. CDK reports a proof with no row as UNSPENT (`crates/cdk/src/mint/check_spendable.rs`), so this proves the originals are released (not stuck PENDING, not SPENT). It does not by itself prove CDK compensated them rather than never recording them. Restart persistence is a row-count claim and is not reconciled through NUT-07. The swap performed after the restart is.

**Verifier:** `npm run verify:nut03-evidence` recomputes every conclusion from the raw values in the files rather than trusting each file's own `pass` field. It exits non-zero on a missing file, `pass=false`, a conservation break, row mismatches, incomplete receipts, rows from a failed swap, partial rows after the crash, restore duplicates, a restart state change, outstanding-liability drift, a contradicting NUT-07 state, the retired crash label, secret-bearing keys, or mixed run ids. `tests/cli/nut03-evidence-verifier.test.ts` proves each of those failure paths against synthetic input in an OS temp directory, never under `evidence/`.

**Not yet verified:** none of the scenario scripts or new workflow steps have run against a real stack. They need GitHub-hosted CI, and pushing was not authorized for this pass. Until an authorized run succeeds, no `nut03-*.json` exists for any real run.

---

## 2026-09-24 — Phase 2 continuation: real NUT-03 swap accounting

Extends Step 8's NUT-04 architecture to real NUT-03 swaps, plus two Step 8 cleanup items: the crash-drill result is relabelled from the informal (and technically wrong) "CRASH_AFTER_COMMIT: RECOVERED" to `CRASH_BEFORE_COMMIT_ROLLBACK` everywhere it's described, and machine-readable evidence for the **NUT-04 (Step 8)** work (`evidence/real-pol/<run-id>/*.json`, 14 files, written by `pol-generate-evidence.ts`) is now generated from the real database at the end of the same real CI run, closing the gap the prior report admitted. **Correction (2026-09-25):** those 14 files are NUT-04-scoped only; this entry originally did not say so, and the NUT-03 work below had **no** dedicated machine-readable evidence until the evidence-correction entry above.

**Real source audit, function-by-function** (`docs/cdk-integration-seams.md`'s NUT-03 transaction map): `setup_swap()` (TX1) inserts input proofs (`state='UNSPENT'`, `operation_kind='swap'`) and output blinded messages (`c=NULL`, same `operation_kind`) from a real `Operation`, then commits — both rows already carry the real operation kind at insertion, never re-derived later. `sign_outputs()` calls `blind_sign()` and (this continuation's addition) `sign_pol_receipt()` per output, both stateless, before any transaction. `finalize()` (TX2) writes the real signatures (firing the *existing* NUT-04 issued-liability trigger automatically — confirmed, not assumed, since `add_blind_signatures()` is the same real write both paths use), persists each receipt's `pending`→`signed` transition in the same transaction (this continuation's addition, mirroring NUT-04 exactly), then transitions the input proofs to `SPENT` — the authoritative consumed-liability event, captured by a new trigger (`migrations/solvent-accounting/0002_nut03_consumed_liability.sql`) — and commits.

**Two real, pre-existing CDK crash-recovery mechanisms this continuation did not need to build**: `SwapSaga::compensate_all()`/`RemoveSwapSetup` (in-process failure) and `Mint::recover_from_incomplete_sagas()` (startup recovery for a crash between TX1 and TX2 — real CDK code, already called from `Mint::start()`). Both were found by tracing, not assumed to exist by analogy with NUT-04.
...
**Real NUT-03 verification suite** (`src/cli/real-cashu/pol-swap-*.ts`): a real swap with real conservation-equation and receipt checks; a real failed-swap/double-spend-after-swap test (the same real scenario satisfies both); a real swap crash drill (genuine `kill -9` inside `finalize()`, before commit); real NUT-09 `POST /v1/restore`-based response-loss recovery (CDK's own existing endpoint, not a new one); multiple consecutive real swaps proving outstanding liability doesn't drift; swap-scoped restart persistence plus one more real swap after restart. Every check passed for real, every step green: [run 36008787769](https://github.com/TheWeirdDee/solvent/actions/runs/36008787769) (2026-09-24) — recorded in that run's CI step logs only. That run's `evidence/real-pol/36008787769/` artifact contains NUT-04 evidence only; no NUT-03-specific machine-readable file exists for it (see the evidence-correction entry above). Two real bugs were found and fixed along the way, by real CI failures, not by inspection: (1) `pol-swap-restore.ts` originally snapshotted "before" row counts *before* the swap itself rather than before the `restore()` call, so the real swap's own real accounting rows were misread as if `restore()` had created them — fixed by moving the snapshot to the correct boundary, then re-confirmed for real (`before={"consumed":24,"issued":24} after={"consumed":24,"issued":24}`); (2) `pol-generate-evidence.ts`'s NUT-04 cross-layer-count query originally scoped to `operation_kind IN ('mint','batch_mint')`, which swept in 3 synthetic `pending`-receipt rows seeded for the recovery test (tagged `batch_mint` specifically to stay out of the real `'mint'` bucket) — fixed by scoping strictly to `'mint'`, matching the already-proven `pol-count-mint-rows.ts` convention; re-confirmed for real (`cdk_blind_signature=48, solvent_issued_liability=48, solvent_signed_receipts=48, all_equal=true`). A third, unrelated CI infrastructure failure (`/tmp/cdk-src` already existed on the runner) was also hit and fixed defensively (`rm -rf` before clone) — not a code regression, but a real observed failure mode worth guarding against regardless of cause.

**Deferred, named explicitly, not silently skipped**: a keyset-rotation swap test (old keyset consumed → new keyset issued) — the user's own instruction permits deferring this specific case if it's too invasive for this milestone; not attempted this pass, not faked. NUT-05 (melt) accounting: not started, per explicit instruction.
