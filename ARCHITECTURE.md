...
## Where these two meet (Phase 2 — NUT-04 built, NUT-03 continuation, NUT-05 not yet)

`verifySubmission()` still accepts a `SubmissionBundle` — raw proof + signed receipt + signed manifest + reserve attestation + Nostr event — from **any** source; nothing in it assumes SOLVENT's own fixture mint produced it. Phase 1 proved the *base Cashu layer* (issue/swap/melt) can be real. Phase 2's job is to make the *SOLVENT-specific layer* (signed PoL receipts, epoch manifests, sum-MMR inclusion, reserve attestation, Nostr publication) also originate from real mint-side activity — modifying the actual mint transaction boundary so a real CDK mint's real events create durable PoL accounting, rather than SOLVENT computing that layer itself from a freshly-generated identity. For NUT-04 (mint issuance) this is real and proven: a small, checked-in patch series adds real receipt signing, crash-durable recovery, and a real retrieval endpoint to a real CDK mint. This continuation extends the same architecture to NUT-03 (swap): a real patch (`patches/cdk/0006-*.patch`) wires consumed-liability, replacement issued-liability, and receipt signing into CDK's real swap transaction, reusing the existing issued-liability trigger — see `docs/receipt-lifecycle.md`, `docs/cdk-signatory-audit.md`, `docs/cdk-integration-seams.md`'s NUT-03 transaction map, and `docs/REALITY-MAP.md`'s Phase 2 table. NUT-05 (melt) accounting has not been started — see `DECISIONS.md`.
